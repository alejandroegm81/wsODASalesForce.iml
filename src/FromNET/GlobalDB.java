...
public class GlobalDB
  ...
  public SubDataTable getQuery(String vQuery, ConnectTo vConnecTo) {
    ...
    try {
      Con = getODADBConnection(vConnecTo);
  ...
  public SubDataTable getQuery(String vQuery, GlobalDBParamObjectList vParams, ConnectTo vConnecTo) {
    ...
    try {
      ...
      for (int k = 0; k < pm.getParameterCount(); k++) {
        if (k < vParams.Count())
          switch (pm.getParameterType(k + 1)) {
            case -16:
            case -1:
            case 1:
            case 12:
            ...
            case 2:
            case 3:
              st.setBigDecimal(k + 1, BigDecimal.valueOf(Long.valueOf((vParams.get(k)).value.toString()).longValue()));
              break;
            case -7:
              st.setBoolean(k + 1, Boolean.valueOf((vParams.get(k)).value.toString()).booleanValue());
              break;
            case -6:
              st.setByte(k + 1, Byte.valueOf((vParams.get(k)).value.toString()).byteValue());
              break;
            case 5:
              st.setShort(k + 1, Short.valueOf((vParams.get(k)).value.toString()).shortValue());
              break;
            case 4:
              st.setInt(k + 1, Integer.valueOf((vParams.get(k)).value.toString()).intValue());
              break;
            case -5:
              st.setLong(k + 1, Long.valueOf((vParams.get(k)).value.toString()).longValue());
              break;
            case 7:
              st.setFloat(k + 1, Float.valueOf((vParams.get(k)).value.toString()).floatValue());
              break;
            case 6:
            case 8:
              st.setDouble(k + 1, Double.valueOf((vParams.get(k)).value.toString()).doubleValue());
              break;
          }
  ...
  public SubDataTable getQuery(String vQuery, OracleConnection ORACon) {
    ...
    try {
      st = ORACon.createStatement(1004, 1007);
  ...
  public SubDataTable setQuery(PreparedStatement prep) {
    ...
    try {
      try {
        ...
        dCol.name = "response";
        dCol.colname = "response";
        dCol.value = Integer.valueOf(res);
        dCol.value_string = String.valueOf(res);
        dRow.cols.add(dCol);
        dData.rows.add(dRow);
        dt.Assign(Boolean.valueOf(true), dData);
      } catch (Exception ex) {
        setErrMsg(ex);
        dt.vDBMessage = ex.getMessage();
      }
      //finally {
        ...
        for (int k = 0; k < pm.getParameterCount(); k++) {
          if (k < vParams.Count()) {
            ...
            try {
              ...
              p("Error:GetParameterType:" + ePP.getMessage());
              vParameterType = -1;
            }
  ...
  public SubDataTable getQuery(PreparedStatement prep) {
    ...
    try {
      try {
        ...
        dCol.name = "response";
        dCol.colname = "response";
        dCol.value = Boolean.valueOf(res);
        dCol.value_string = String.valueOf(res);
        dRow.cols.add(dCol);
        dData.rows.add(dRow);
