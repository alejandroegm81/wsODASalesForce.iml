...
public class wsOperaciones_MOVIL extends BaseClass {


    String getXmlQuery(wsInstancias.wsInstancia Instancia){
        String vQuery = "";
        try{
            String vConfFile = "MOVIL_" + getCountryCode(Instancia) + ".xml";
    ...
    public wsR_gBusqueda Busqueda(  wsInstancias.wsInstancia Instancia,
        ...
        if( !OperacionServicio.equals( wsOperacionServicios.wsOperacionServicio.MOVIL ) ){ throw new Exception("Operacion no es Busqueda Movil"); }
            ...
            vQuery = getXmlQuery(Instancia);if(vQuery==""){ throw new Exception("No se encontro archivo de configuracion"); }
            ...
            if(dt.vData && dt.vRows>1){
                ...
                for (int i = 1; i < dt.vRows; i++) {
                    ...
                    if(vCFound==false){//Agregar Cliente
                        if( dt.Datos.rows.get(i).getColByName("DN_NUM").value_string!=null && dt.Datos.rows.get(i).getColByName("DN_NUM").value_string.length()>0){
                            if( dt.Datos.rows.get(i).getColByName("DN_NUM").value_string.substring(1,3).equals("00") && dt.Datos.rows.get(i).getColByName("DN_NUM").value_string.length()>11 ){
                                vCliente.OperacionServicio  = wsOperacionServicios.wsOperacionServicio.FIJO;
                            }else{
                                vCliente.OperacionServicio  = wsOperacionServicios.wsOperacionServicio.MOVIL;
                            }
                        }else{
                            vCliente.OperacionServicio  = wsOperacionServicios.wsOperacionServicio.MOVIL;
                        }
                        ...
                        if(vCliente.Telefono!=null){
                            vCliente.Telefono = (vCliente.Telefono.length()>8)?vCliente.Telefono.substring(3):vCliente.Telefono;
                        }
                        ...
                        vCliente.SaldoTotal	        =	0.0; //dt.Datos.rows.get(i).getColByName("SALDO_TOTAL").value_double;
                        vCliente.SaldoNoVencido     =	0.0; //dt.Datos.rows.get(i).getColByName("SALDO_NOVENCIDO").value_double;
                        vCliente.SaldoVencido	    =	0.0; //dt.Datos.rows.get(i).getColByName("SALDO_VENCIDO").value_double;
